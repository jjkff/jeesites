...
<div id="importBox" class="hide">
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/sierac/distributor/">经销商列表</a></li>
		<shiro:hasPermission name="sierac:distributor:edit">
			<li><a href="${ctx}/sierac/distributor/formAdd">经销商添加</a></li>
		</shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="distributor" action="${ctx}/sierac/distributor/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>是否启用</th>
				<th>增加时间</th>
				<shiro:hasPermission name="sierac:distributor:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="distributor">
			<tr>
				<td>
					<%--<a href="${ctx}/sierac/distributor/form?id=${distributor.id}">--%>
					${distributor.num}
				</a></td>
				...
				<td>
					${distributor.isEnable == '1' ? '启用': '不启用'}
				</td>
				<td>
					${distributor.myCreateDate}
				</td>
				<shiro:hasPermission name="sierac:distributor:edit"><td>
					<a href="${ctx}/sierac/distributor/form?id=${distributor.id}">查看详情</a>
    				<%--<a href="${ctx}/sierac/distributor/formUp?id=${distributor.id}">修改</a>--%>
					<%--<a href="${ctx}/sierac/distributor/delete?id=${distributor.id}" onclick="return confirmx('确认要删除该经销商吗？', this.href)">删除</a>--%>
